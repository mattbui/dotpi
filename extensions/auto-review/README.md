...
The reviewer uses `openai-codex/gpt-5.3-codex-spark` with low reasoning by default, receives no tools, and does not set an explicit max output token cap. It only returns JSON: allow/deny/escalate_to_user, risk, and rationale.
